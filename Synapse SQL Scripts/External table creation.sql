...
CREATE DATABASE SCOPED CREDENTIAL cred_sakshi
WITH 
...
WITH (
    LOCATION = 'https://de1storagedatalake.dfs.core.windows.net/silver', -- URL to the external data storage location
    CREDENTIAL = cred_sakshi, -- Use the 'cred_sakshi' credential for authentication
);

-- Create another external data source named 'source_gold', pointing to the 'gold' directory in Azure Data Lake Storage.
CREATE EXTERNAL DATA SOURCE source_gold
WITH (
    LOCATION = 'https://de1storagedatalake.dfs.core.windows.net/gold', -- URL to the external data storage location
    CREDENTIAL = cred_sakshi, -- Use the 'cred_siva' credential for authentication
);
...
AS
SELECT * FROM gold.sales; -- Select all data from the 'gold.sales' view and load into the external table

SELECT * FROM gold.extsales; --Select data from external table to verify the creation
